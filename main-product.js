...
document.addEventListener("DOMContentLoaded", function () {
  function addProductEntry(productJson, storedProducts) {
    ...
    if (!storedProducts.includes(currentProductID)) {
      ...
      localStorage.setItem(
        "milano__recently-viewed-products",
  ...
  if (prodData != null) {
    ...
    var storedProducts = JSON.parse(
      localStorage.getItem("milano__recently-viewed-products")
    );
...
function runVariantImageFilter() {
  console.log("🧠 Variant image filter JS is running");
  ...
  function updateGalleryByColor(selectedColor) {
    const productData = getProductData();
    if (!productData || !selectedColor) return;

    const filteredMedia = productData.media.filter((media) => {
      return (
        media.alt &&
        media.alt.toLowerCase().trim() === selectedColor.toLowerCase().trim()
      );
    });

    console.log("🎯 Filtered images:", filteredMedia);

    // Swiper view (mobile/lightbox)
    document.querySelectorAll(".swiper-slide").forEach((slide) => {
      const img = slide.querySelector("img");
      if (!img) return;
      const alt = img.alt.trim().toLowerCase();
      slide.style.display =
        alt === selectedColor.toLowerCase() ? "block" : "none";
    });

    // Desktop grid view
    document
      .querySelectorAll(".main-gallery--desktop .media-gallery_image")
      .forEach((item) => {
  ...
  if (productForm && productJson) {
    ...
    if (initialVariant) {
      const colorOptionIndex = productData.options.findIndex(
        (opt) => opt.toLowerCase() === "color"
      );
      if (colorOptionIndex > -1) {
        const initialColor = initialVariant.options[colorOptionIndex];
        updateGalleryByColor(initialColor);
      }
    }
  }

  // Listen to swatch click
  document.addEventListener("click", function (e) {
    const swatch = e.target.closest(".product__color-swatches--js");
    if (swatch) {
      const color =
        swatch.getAttribute("data-swatch-value") || swatch.textContent.trim();
      updateGalleryByColor(color);
    }
  });
}

// Run on first load
document.addEventListener("DOMContentLoaded", runVariantImageFilter);

// 🧠 Run again if Shopify reloads section (on variant switch)
document.addEventListener("shopify:section:load", runVariantImageFilter);
...
document.addEventListener('DOMContentLoaded', function () {
  const colorInputs = document.querySelectorAll(
    'select[name*="olor"], input[name*="olor"], .product__color-swatches--js input'
  );

  colorInputs.forEach(function (el) {
    el.addEventListener('change', function () {
      const selectedValue = el.value || el.getAttribute('value');

      // Simulate changing variant via Shopify's own variant picker
      const variantRadios = document.querySelector('variant-radios');
      const variantSelects = document.querySelector('variant-selects');

      if (variantRadios && typeof variantRadios.updateVariant === 'function') {
        variantRadios.updateVariant();
      } else if (variantSelects && typeof variantSelects.updateVariant === 'function') {
        variantSelects.updateVariant();
      }

      // Optional fallback if variant objects aren't found
      const form = el.closest('form');
      if (form) {
        const event = new Event('change', { bubbles: true });
        form.querySelector('select[name="id"]')?.dispatchEvent(event);
      }
    });
  });
});
...
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(() => {
    const pickers = document.querySelectorAll('f-variant-picker, x-variant-picker');
